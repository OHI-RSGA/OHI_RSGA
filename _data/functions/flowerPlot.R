...
PlotFlower <- function(region_plot     = NA,
                       ...
                       dir_fig_save    = here("chapters/figs/FlowerPlots"),
  ...
  if ( !any(is.na(region_plot)) ){
    scores <- scores %>%
      filter(region_name %in% region_plot)
  }
  ...
  region_plots <- unique(scores$region_name) 

  
  ## goals.csv configuration info----
  
  ## read in conf/goals.csv, start dealing with supra goals
  conf <-  readr::read_csv("https://raw.githubusercontent.com/OHI-Science/ohi-global/draft/eez/conf/goals.csv")
  ...
  region_names <- region_names_all %>%
    filter(region_name %in% region_plots) %>%  ## filter only regions to plot
    distinct()                              ## in case region_id 0 was included in regions_list.csv
  
  
  ## loop through to save flower plot for each region ----
  for (region in region_plots) { # region = 'Global average'
    ...
    ## fig_name to save
    fig_save <- region_names$flower_png[region_names$region_name == region]
    ...
    plot_obj <- plot_obj +
      geom_bar(aes(y = 100),
               stat = 'identity', color = light_line, fill = white_fill, linewidth = .2) +
      geom_errorbar(aes(x = pos, ymin = 100, ymax = 100, width = weight),
                    linewidth = 0.5, color = light_line, show.legend = NA)
    ...
    plot_obj <- plot_obj +
      ...
      geom_errorbar(aes(x = pos, ymin = score, ymax = score),
                    linewidth = 0.5, color = dark_line, show.legend = NA) +
    ...
    plot_obj <- plot_obj +
      geom_text(data = plot_score_index,
                inherit.aes = FALSE,
                aes(label = score),
